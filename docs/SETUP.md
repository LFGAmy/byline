...
- Back in **Environment Variables**, add `BYLINE_TOKENS` with the value `thatcode:Client Name`. The name is whatever you want on the stamp and in the email subject: a company (`Acme Team`) if several people there will open it, a person if it's one reader. **The name is required.** A code with nothing after the colon, or no colon at all, makes every page show "This document is not available right now" until you fix it. One code per reader, for as long as they should have access. To add a reader, make another code and add it to the same line, separated by a comma: `code1:Acme,code2:Globex`. The existing ones keep working.
